...
{{- if $titanSideCars }}
  ...
  {{- if $gatewayEnable }}
    {{- range $cn, $cv := $clusters }}
      {{- if and (ne $cn "local-myapp") (ne $cn "remote-myapp") }}
        {{- $ratelimitRoutes := $cv.routes }} 
        {{- if $cv.ratelimit }}
          {{- if $cv.ratelimit.routes }}
            {{- $ratelimitRoutes = $cv.ratelimit.routes }} 
          {{- end }}
        {{- end }}
        {{- range $ratelimitRoutes }}
  ...
  {{- else }}
    ...
    {{- if and $ingress (not $routes) }}
      ...
      {{- end }}
    {{- end }}
    {{- $foundRatelimitRoutes := false }}
    {{- if $ingress.ratelimit }}
      {{- if $ingress.ratelimit.routes }}
        {{- $routes = $ingress.ratelimit.routes }}
        {{- $foundRatelimitRoutes = true }}
      {{- end }}
    {{- end }}
    {{- if not $foundRatelimitRoutes }}
      {{- if $localApp.ratelimit }}
        {{- if $localApp.ratelimit.routes }}
          {{- $routes = $localApp.ratelimit.routes }}
        {{- end }}
      {{- end }}
    {{- end }}
